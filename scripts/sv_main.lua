...
function txaSendDM(source, args)
    if args[1] ~= nil and args[2] ~= nil and args[3] ~= nil then
        ...
        if pName ~= nil then
            ...
            TriggerClientEvent('chat:addMessage', -1, { 
                template = '<div style="padding: 1.0vw; margin: 1.0vw; background-color: rgba(255, 149, 0, 0.4); border-radius: 5px;"><i class="fas fa-crown"></i> {0}:<br> {1}</div>', --{0}:<br> {1}</div>
                args = { "(DM) "..args[2], args[3] }, color = {255, 0, 0} -- 128, 128, 128
            })
